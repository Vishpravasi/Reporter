...
function LoadViewDetails() {
    ...
    $.ajax({
        url: gStrIpVal + "/NewsStreamService.svc/ViewDetails",
        cache: false,
        type: "POST",
        async: false,
        data: '{"IssueDate": "' + IssueDate + '","Product": "' + Product + '","LayoutDesk": "' + LayoutDesk + '","ContentDesk": "' + ContentDesk + '","Zone": "' +
         Zone + '","Page": "' + Page + '","strUserId": "' + strTmpUserId + '","strRefStoryName": "' + $('#txtRefStoryName').val() + '","strArtUserID": "' + $('#ddlAllUserList').val() + '"}',
        ...
        success: function (data) {
            ...
            if (data.ViewDetailsResult.length > 0) {
                $.each(data.ViewDetailsResult, function (index, value) {
                    if ((data.ViewDetailsResult[index].strstatus == "T" && data.ViewDetailsResult[index].strArticles_Created == window.sessionStorage.strUserId) || data.ViewDetailsResult[index].strstatus != "T") {
                        var strStyleBkColor = "";
                        if (data.ViewDetailsResult[index].strChiefRepoterViewed == "Y")
                            strStyleBkColor = "style='background-color: #FFFACD;'";
                        if (data.ViewDetailsResult[index].strEditorViewed == "Y")
                            strStyleBkColor = "style='background-color: #AFEEEE;'";
                        if (data.ViewDetailsResult[index].strSubEditorViewed == "Y")
                            strStyleBkColor = "style='background-color: #D8BFD8;'";

                        strVal += "  <tr title='" + data.ViewDetailsResult[index].strcontent + "' " + strStyleBkColor + "> ";
                        strVal += " <td class='strParentId'>" + data.ViewDetailsResult[index].ParentId + "</td> ";
                        strVal += " <td class='StoryRef'>" + data.ViewDetailsResult[index].strRef_story_name + "</td> ";
                        strVal += " <td style='display:none' class='Caption'>" + data.ViewDetailsResult[index].Caption + "</td> ";
                        strVal += " <td class='Zone'>" + data.ViewDetailsResult[index].Zone + "</td> ";
                        strVal += " <td class='IssueDate'>" + data.ViewDetailsResult[index].IssueDate + "</td> ";
                        strVal += " <td class='Page'>" + data.ViewDetailsResult[index].strPageName + "</td> ";
                        if (data.ViewDetailsResult[index].strstatus != "A") {
                        ...
                        strVal += " <td class=''>" + data.ViewDetailsResult[index].strCreatedUser + "</td> ";
                        if (window.sessionStorage.strUserGroup == "RPT") {
                            strVal += " <td class=''>" + data.ViewDetailsResult[index].strReportUser + "</td> ";
                            strVal += " <td class=''>" + data.ViewDetailsResult[index].strChiefReportUser + "</td> ";
                            strVal += " <td class=''>" + data.ViewDetailsResult[index].strEditorialUser + "</td> ";
                            strVal += " <td class=''>" + data.ViewDetailsResult[index].strSubEditorialUser + "</td> ";
                        }
                        ...
                        if (window.sessionStorage.strUserGroup == "EDTSUP") {
                            strVal += " <td class=''>" + data.ViewDetailsResult[index].strSupplementary_Editor + "</td> ";
                            //strVal += " <td class=''>" + data.ViewDetailsResult[index].strSubEditorialUser + "</td> ";
                        }
                        //                    }
                        //                    else {
                        ...
                        //                    }
                        strVal += " <td>";
                        if (data.ViewDetailsResult[index].strstatus != "A" && window.sessionStorage.strUserGroup != "SUP" && (data.ViewDetailsResult[index].strProcessedUser == "" || data.ViewDetailsResult[index].strProcessedUser == window.sessionStorage.strUserId)) {
                            strVal += " <a class='btn btn btn-primary' href='ArticleEditor.html?ArticleId=" + data.ViewDetailsResult[index].ParentId + "&Display=EDIT'>Edit </a>";
                        }
                        //                    if ((window.sessionStorage.strUserGroup == "SUP" || window.sessionStorage.strUserGroup == "EDT" || window.sessionStorage.strUserGroup == "CHRPT" || window.sessionStorage.strUserGroup == "SUBEDT") && data.ViewDetailsResult[index].strstatus != "A") {
                        var strProcessAnotherUser = "";
                        if ((data.ViewDetailsResult[index].strProcessedUser != "" && data.ViewDetailsResult[index].strProcessedUser != window.sessionStorage.strUserId)) {
                            strProcessAnotherUser = " title='The story cannot modify because it is being used by " + data.ViewDetailsResult[index].strProcessedUser + " user ' ";
                        }
                        strVal += " <a class='btn btn btn-primary' " + strProcessAnotherUser + "  href='ArticleEditor.html?ArticleId=" + data.ViewDetailsResult[index].ParentId + "&Display=View'>View </a>";
                        if (window.sessionStorage.strUserGroup != "SUP")
                            strVal += " <a class='btn btn btn-primary' id='" + data.ViewDetailsResult[index].ParentId + "' title='" + data.ViewDetailsResult[index].strRef_story_name + "' onclick='DeleteStory(this.id, this.title)' >Delete </a>";
                        //                    }
                        strVal += "  </td>";
                        strVal += " </tr> ";
                    }
