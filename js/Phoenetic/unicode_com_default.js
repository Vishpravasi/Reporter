...
function isSplKeyy(keynum) {
    retVal = false;
    for (i in SplKeyys) {
        if (keynum == SplKeyys[i])
            retVal = true;
    }
    return retVal;
}
...
function convertThisgen(e, numchar) {
    if (!isIE)
        Key = e.which;
    else
        Key = e.keyCode;
